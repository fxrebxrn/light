...
def queues_kb(action_type, company, lang):
    ...
    btns = [types.InlineKeyboardButton(q, callback_data=cb_sched.new(comp=company, queue=q) if action_type == 'view' else cb_menu.new(action='save', val=f"{company}:{q}")) for q in queues]
    kb.add(*btns)
...
async def my_queues(message: types.Message):
    ...
    kb = types.InlineKeyboardMarkup()
    for r in rows: kb.add(types.InlineKeyboardButton(f"❌ {r['company']} {r['queue']}", callback_data=f"del_{r['id']}"))
...
def register_handlers(dp: Dispatcher):
    ...
    dp.register_message_handler(view_schedules_start, lambda m: "рафік" in m.text)
    dp.register_message_handler(add_queue_btn, lambda m: "одати" in m.text)
    dp.register_message_handler(my_queues, lambda m: "ої чер" in m.text)
    
    # Callback-и (Компанії)
    dp.register_callback_query_handler(handle_comp_selection, lambda c: c.data.startswith(('v_comp:', 's_comp:')))
